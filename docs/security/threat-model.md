...
SFTP reuses the verified SSH transport on a separate subsystem channel. The renderer has no local path or byte-payload command. Native selection creates an opaque typed grant in Rust, and an immutable backend plan binds it to one host/connection/subsystem, exact endpoints, observed identities, conflict choice, risk, expiry, and one execution. These checks limit confused-deputy and stale-session use; they cannot prove a human saw an honest prompt after renderer compromise.
...
The remote server, local filesystem, or another writer can race metadata checks. SFTP v3 does not provide a universal compare-and-swap operation, and path-based local finalization cannot defend against a malicious logged-in account or filesystem driver. No-clobber primitives, exact staging ownership, final revalidation, and destination serialization reduce accidental races. `OutcomeUnknown` is used when a lost reply prevents an honest result, and retry always creates another plan.
