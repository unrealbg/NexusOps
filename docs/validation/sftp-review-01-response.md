...
The first hosted macOS run exposed a test-fixture portability error: `tempfile::tempdir()` selected `/var/folders`, while the product's ancestry policy correctly rejects macOS `/var` because it is a symlink. The failing core test exited 101 before retry I/O. Disposable local-access/SFTP test directories were moved under the checked-out repository, whose ancestry is ordinary on all CI platforms. Product behavior was not relaxed. The full local Rust unit/bin suite, format and Clippy then passed again before the correction was pushed.
